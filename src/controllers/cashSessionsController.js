...
async function openCashSession(request, reply) {
  ...
  try {
    const session = await cashSessionsService.openSession({
      ...
      openingVarianceReason: parsed.data.openingVarianceReason,
    });
    ...
    if (e.code === "OPENING_VARIANCE_REASON_REQUIRED") {
...
async function closeCashSession(request, reply) {
  ...
  try {
    const session = await cashSessionsService.closeSession({
      ...
      closingVarianceReason: parsed.data.closingVarianceReason,
    ...
    if (e.code === "CLOSING_VARIANCE_REASON_REQUIRED") {
      return reply.status(400).send({ error: e.message });
    }
    request.log.error(e);
    return reply.status(500).send({ error: "Internal Server Error" });
